...
export const SSOSettings: React.FC = () => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const res = await axios.post('/api/admin/test-auth', { 
      ...
      setTestResult({ success: false, message: err.response?.data?.error || 'Network Protocol Error: Could not reach backend.' });
    } finally {
      setTesting(false);
    }
  };
  ...
  const toggleSSO = async (enabled: boolean) => {
    if (!settings) return;
    try {
        await axios.post('/api/admin/toggle-sso', { enabled });
        setSettings({ ...settings, AUTH_PROVIDER: enabled ? 'central' : 'local' });
    } catch (err) {
        console.error("Toggle SSO failed", err);
    }
  };
              ...
              <div className="grid grid-cols-2 gap-4 pt-4">
                <button 
                  onClick={async () => {
                    ...
                    setTesting(true);
                    try {
                      const res = await axios.post('/api/admin/save-auth-settings', { 
                        base_url: settings.CENTRAL_AUTH_SERVER_ADDRESS,
                        client_id: settings.CENTRAL_AUTH_CLIENT_ID,
                        client_secret: clientSecret
                      });
                      ...
                      setTestResult({ success: false, message: err.response?.data?.error || 'Lỗi khi lưu cấu hình.' });
                    } finally {
                      setTesting(false);
                    }
                  }}
