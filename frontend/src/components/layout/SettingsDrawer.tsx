...
import { 
    X, Palette, Save, Check, Clock, 
    Globe, Download, Trash2, RefreshCw, Upload,
    Layers, Languages, Edit3, ChevronLeft,
    Zap, Gauge, MessageSquare, Users, AlignLeft, AlignCenter, AlignRight
} from 'lucide-react';
...
export const SettingsDrawer: React.FC = () => {
    const { 
        ...
        availableTracks, fetchAvailableTracks, translateTrack, exportTrack, updateTrackName,
                    ...
                    <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed top-0 right-0 h-screen w-full max-w-md bg-slate-950 border-l border-white/10 z-[101] flex flex-col shadow-2xl overflow-hidden font-inter">
                            ...
                            <AnimatePresence mode="wait">
                                    ...
                                    <motion.div key="subtitles" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                                        ...
                                        {librarySubTab === 'tracks' && (
                                            <div className="space-y-4">
                                                {availableTracks.map(track => (
                                                    <div key={track.id} className="bg-slate-900/60 rounded-3xl p-5 border border-white/5 space-y-4">
                                                        <div className="flex items-center justify-between">
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    {editingTrackId === track.id ? (
                                                                        <div className="flex items-center gap-1">
                                                                            <input value={editTrackName} onChange={e => setEditTrackName(e.target.value)} className="bg-slate-950 border border-white/10 rounded px-2 py-0.5 text-[10px] outline-none" />
                                                            ...
                                                            <div className="flex gap-1">
                                                                {['s1', 's2', 's3'].map(slot => (
                                                                    <button key={slot} onClick={() => setTrackIds({ [slot]: track.id })}
                                                                        className={`w-8 h-8 rounded-lg text-[9px] font-black flex items-center justify-center transition-all ${trackIds[slot as keyof typeof trackIds] === track.id ? 'bg-sky-500 text-slate-950' : 'bg-white/5 text-slate-500'}`}
                                                                    >
                                                                        T{slot.slice(1)}
                                                                    </button>
                                                                ))}
                                                        ...
                                                        <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                                            <div className="flex gap-2">
                                                                ...
                                                                <button onClick={() => translateTrack(track.id, 'vi', `${track.name} (VN)`)} className="p-2 text-sky-500 bg-sky-500/10 rounded-lg hover:bg-sky-500/20"><Languages size={14} /></button>
                                                                <button onClick={() => exportTrack(track.id, 'srt')} className="p-2 text-slate-400 hover:text-white"><Download size={14} /></button>
                                                            </div>
                                                            <button onClick={() => handleDeleteTrack(track.id)} className="p-2 text-slate-700 hover:text-rose-500"><Trash2 size={14} /></button>
                                                        </div>
                                                ...
                                                <div className="grid gap-3">
                                                    {ytTracks.map(t => (
                                                        <div key={t.lang_code + t.is_auto} className="bg-white/5 border border-white/5 rounded-2xl p-4 flex items-center justify-between">
                                                            ...
                                                            <button onClick={() => handleImport(t.lang_code, t.is_auto)} className="px-4 py-2 bg-sky-500/10 border border-sky-500/20 text-sky-500 rounded-xl text-[10px] font-black uppercase hover:bg-sky-500 hover:text-slate-950 transition-all">Import</button>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        <section className="bg-amber-500/5 rounded-3xl p-6 border border-amber-500/10 space-y-4 mt-8">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-xs font-mono font-bold text-amber-400">{settings.syncOffset > 0 ? '+' : ''}{settings.syncOffset.toFixed(1)}s</span>
                                            </div>
                                            <input type="range" min="-2" max="2" step="0.1" value={settings.syncOffset || 0} onChange={(e) => usePlayerStore.getState().setSyncOffset(parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-800 appearance-none rounded-full accent-amber-500 cursor-pointer" />
                                        </section>
                                    </motion.div>
                                )}
                                    ...
                                    <motion.div key="display" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                                    ...
                                                    <div className="bg-white/5 rounded-[2.5rem] p-6 border border-white/5 space-y-6">
                                                        ...
                                                        <div className="flex items-center justify-between border-t border-white/5 pt-5">
                                                            ...
                                                            <div className="flex items-center gap-4">
                                                                <div className="flex gap-2">
                                                                    <input type="color" value={s.color} onChange={e => update({ color: e.target.value })} title="Text Color" className="w-8 h-8 rounded-full bg-transparent p-0 cursor-pointer border-none" />
                                                                    <input type="color" value={s.bgColor} onChange={e => update({ bgColor: e.target.value })} title="BG Color" className="w-8 h-8 rounded-xl bg-transparent p-0 cursor-pointer border-none" />
                                                                </div>
                                        ...
                                        <section className="bg-amber-500/5 rounded-[2.5rem] p-6 border border-amber-500/10 space-y-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <h3 className="text-xs font-black uppercase text-amber-400">Notes & Flashcards</h3>
                                                    <p className="text-[10px] text-slate-500 font-medium">Auto-display saved items</p>
                                                </div>
                                                <button onClick={() => setNoteSettings({ enabled: !settings.notes.enabled })} className={`ml-auto px-4 py-2 rounded-xl text-[10px] font-black transition-all ${settings.notes.enabled ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-500/20' : 'bg-slate-800 text-slate-500'}`}>
                                            ...
                                            {settings.notes.enabled && (
                                                <div className="space-y-6 animate-in fade-in slide-in-from-top-4">
                                                    ...
                                                    <div className="space-y-4">
                                                        <span className="text-[10px] font-black text-slate-500 uppercase px-1">Display Theme</span>
                                                        <div className="flex bg-slate-900/60 p-1 rounded-2xl border border-white/5 gap-1">
                                                            {(['classic', 'cyber', 'amber', 'ghost'] as const).map(t => (
                                                                <button key={t} onClick={() => setNoteSettings({ theme: t })}
                                                                    className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${settings.notes.theme === t ? 'bg-amber-500 text-slate-950 shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                                                                >
                                                                    {t}
                                                                </button>
                                                            ))}
                                                    ...
                                                    <div className="space-y-4">
                                                        <span className="text-[10px] font-black text-slate-500 uppercase px-1">Screen Alignment</span>
                                                        <div className="grid grid-cols-3 bg-slate-900/60 p-1 rounded-2xl border border-white/5 gap-1">
                                                            {(['topLeft', 'topCenter', 'topRight', 'centerLeft', 'center', 'centerRight', 'bottomLeft', 'bottomCenter', 'bottomRight'] as const).map(pos => (
                                                                <button key={pos} onClick={() => setNoteSettings({ alignment: pos })}
                                                                    className={`py-2 rounded-xl text-[8px] font-black uppercase transition-all ${settings.notes.alignment === pos ? 'bg-amber-500/20 text-amber-500 border border-amber-500/30 shadow-inner' : 'text-slate-600 hover:text-slate-400'}`}
                                                                >
                                                                    {pos.replace(/([A-Z])/g, ' $1')}
                                                                </button>
                                                            ))}
                                                    ...
                                                    <div className="grid grid-cols-2 gap-6 pt-2">
                                                        <div className="space-y-3">
                                                            <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest px-1">Timing (Offset)</label>
                                                            <div className="flex items-center gap-3 bg-slate-900/40 rounded-xl p-2 border border-white/5">
                                                                <Clock size={12} className="text-slate-600" />
                                                                <input type="number" step="0.5" value={settings.notes.beforeSecs} onChange={e => setNoteSettings({ beforeSecs: parseFloat(e.target.value) })} className="bg-transparent w-full text-xs font-bold text-amber-500 outline-none" />
                                                                <span className="text-[9px] font-black text-slate-700">SEC</span>
                                                            </div>
                                                        </div>
                                                        <div className="space-y-3">
                                                            <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest px-1">Stay Duration</label>
                                                            <div className="flex items-center gap-3 bg-slate-900/40 rounded-xl p-2 border border-white/5">
                                                                <Clock size={12} className="text-slate-600" />
                                                                <input type="number" step="0.5" value={settings.notes.duration} onChange={e => setNoteSettings({ duration: parseFloat(e.target.value) })} className="bg-transparent w-full text-xs font-bold text-amber-500 outline-none" />
                                                                <span className="text-[9px] font-black text-slate-700">SEC</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        <section className="bg-emerald-500/5 rounded-[2.5rem] p-6 border border-emerald-500/10 space-y-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <h3 className="text-xs font-black uppercase text-emerald-400">Social Engine</h3>
                                                    <p className="text-[10px] text-slate-500 font-medium">Comments & Community activity</p>
                                                </div>
                                                <button onClick={() => setCommunitySettings({ enabled: !settings.community.enabled })} className={`ml-auto px-4 py-2 rounded-xl text-[10px] font-black transition-all ${settings.community.enabled ? 'bg-emerald-500 text-slate-950 shadow-lg shadow-emerald-500/20' : 'bg-slate-800 text-slate-500'}`}>
                                            ...
                                            {settings.community.enabled && (
                                                <div className="space-y-6 animate-in fade-in slide-in-from-top-4">
                                                    <div className="space-y-4">
                                                        <span className="text-[10px] font-black text-slate-500 uppercase px-1">Display Mode</span>
                                                        <div className="flex bg-slate-900/60 p-1 rounded-2xl border border-white/5 gap-1">
                                                            {[
                                                                { id: 'danmaku', label: 'Barrage', icon: <Zap size={10} /> },
                                                                { id: 'fixed', label: 'Static', icon: <MessageSquare size={10} /> }
                                                            ].map(m => (
                                                                <button key={m.id} onClick={() => setCommunitySettings({ mode: m.id as any })}
                                                                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${settings.community.mode === m.id ? 'bg-emerald-500 text-slate-950' : 'text-slate-500 hover:text-slate-300'}`}
                                                                >
                                                                    {m.icon} {m.label}
                                                                </button>
                                                            ))}
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <StyleSlider label="Text Size" val={`${(settings.community.fontSize * 10).toFixed(0)}%`} min={1} max={5} step={0.1} value={settings.community.fontSize} onChange={v => setCommunitySettings({ fontSize: v })} />
                                                        <StyleSlider label="Opacity" val={`${(settings.community.opacity * 100).toFixed(0)}%`} min={0.1} max={1} step={0.05} value={settings.community.opacity} onChange={v => setCommunitySettings({ opacity: v })} />
                                                    </div>
                                                </div>
                                            )}
                                        </section>
                                ...
                                {activeTab === 'vocab' && (
                                    <motion.div key="vocab" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                        <div className="space-y-4">
                                            <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500"><Globe size={12} className="text-amber-500" /> Source Subtitle Track</label>
                                            <select value={sourceTrackId || ''} onChange={e => setSourceTrackId(Number(e.target.value))} className="w-full bg-slate-900 border border-white/10 rounded-2xl px-6 py-4 text-xs font-bold text-white outline-none focus:border-amber-500/50">
                                                <option value="" disabled>Select study track...</option>
                                        ...
                                        <div className="p-6 rounded-[2.5rem] bg-slate-900/40 border border-white/5 flex items-center justify-between">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <p className="text-[9px] text-slate-500 font-medium leading-tight max-w-[180px]">AI splitting for word-by-word learning</p>
                                            </div>
                                        ...
                                        <div className="space-y-4">
                                            <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Dictionary Language</label>
                                            <div className="grid grid-cols-3 gap-2">
                                                {['vi', 'en', 'cn'].map(lang => (
                                                    <button key={lang} onClick={() => setTargetLang(lang as any)}
                                                        className={`px-3 py-3 rounded-2xl border text-[10px] font-black uppercase transition-all ${targetLang === lang ? 'bg-amber-500/10 border-amber-500/40 text-amber-500' : 'bg-white/5 border-white/5 text-slate-500'}`}
                                                    >
                                                        {lang.toUpperCase()}
                                                    </button>
                                                ))}
                                        ...
                                        <div className="space-y-4">
                                            <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Dictionary Override</label>
                                            <select value={preferredSystemDictId || ''} onChange={e => setPreferredSystemDictId(e.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-2xl px-6 py-4 text-xs font-bold text-white outline-none">
                                                <option value="">Default (Auto)</option>
                                                {globalDictionaries.filter(d => {
                                                    const selected = availableTracks.find(t => t.id === sourceTrackId);
                                                    return d.src === (selected?.language_code || 'ja') && d.target === targetLang;
                                                }).map(d => (
                                                    <option key={d.id} value={d.id}>{d.name} ({d.count} terms)</option>
                                                ))}
                                            </select>
                                        </div>
                                    </motion.div>
                        ...
                        <div className="shrink-0 p-6 bg-slate-950/80 backdrop-blur-xl border-t border-white/5 space-y-3">
                            <div className="flex gap-3">
                                {activeTab === 'vocab' ? (
                                    <button 
                                        onClick={handleApplyVocabSettings} 
                                        disabled={isSaving}
                                        className="flex-1 py-4 bg-amber-500 text-slate-950 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-lg shadow-amber-500/20 active:scale-95 disabled:opacity-50"
                                    >
                                        {saveStatus === 'success' ? 'VOCAB UPDATED!' : isSaving ? 'APPLYING...' : 'APPLY & REFRESH'}
                                    </button>
                                ) : (
                                    <button 
                                        onClick={async () => {
                                        ...
                                        disabled={isSaving || activeTab === 'hub'}
                                        className="flex-1 py-4 bg-sky-500 text-slate-950 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-lg shadow-sky-500/20 active:scale-95 disabled:opacity-50"
                                    >
                                        {saveStatus === 'success' ? 'LESSON SAVED!' : isSaving ? 'SAVING...' : 'SAVE FOR THIS LESSON'}
                                    </button>
                                )}
                                ...
                                {activeTab !== 'hub' && (
                                    <button 
                                        ...
                                        className="p-4 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-2xl transition-all active:scale-95 disabled:opacity-50"
                                        title="Save as Global Template"
                                    >
                                        {globalSaveStatus === 'success' ? <Check size={16} className="text-emerald-500" /> : <Save size={16} />}
                                    </button>
                                )}
                            </div>
                            <p className="text-[8px] text-center font-black text-slate-700 uppercase tracking-widest">
                                {activeTab === 'vocab' ? 'Requires re-analysis of the current text' : (globalSaveStatus === 'success' ? 'Settings synced to cloud' : 'Changes apply instantly to current view')}
                            </p>
                        </div>
                    </motion.div>
                </>
