...
export const OverviewPanel: React.FC = () => {
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden h-full">
            {/* ─── TAB BAR ─── */}
            <div className="px-2 md:px-4 pb-1.5 md:pb-3 flex gap-2 items-center shrink-0">
                {!useFullDropdownMode ? (
                ...
                {isAdmin && (
                    <div className="flex items-center gap-2">
                        ...
                        {/* Mobile-only Edit Controls */}
                        <div className="md:hidden flex items-center gap-2">
                            {isEditing ? (
                                <>
                                    <button 
                                        onClick={handleCancelEdit}
                                        className="p-2.5 bg-slate-800 text-slate-400 rounded-xl border border-white/5"
                                        title="Hủy"
                                    >
                                        <X size={16} />
                                    </button>
                                    <button 
                                        ...
                                        className="p-2.5 bg-emerald-500 text-white rounded-xl shadow-lg shadow-emerald-500/20"
                                        title="Lưu"
                                    >
                                        <Save size={16} />
                                    </button>
                                ...
                                <button 
                                    ...
                                    className="p-2.5 bg-sky-500 text-white rounded-xl shadow-lg shadow-sky-500/20"
                                    title="Chỉnh sửa"
                                >
                                    <Edit2 size={16} />
                                </button>
            ...
            {/* ─── CONTENT AREA ─── */}
            <div className="flex-1 overflow-y-auto px-4 md:px-6 pt-0 md:pt-4 pb-0 md:pb-8 custom-scrollbar">
                {isEditing ? (
...
const SubTabButton: React.FC<{
    ...
    return (
        <div className="flex items-center gap-1">
            <button
                onClick={onClick}
                className={`flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-xl border text-[9px] md:text-[10px] font-bold uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${active ? `${colors[color]} shadow-lg shadow-${color}-500/10` : 'bg-transparent border-white/5 text-slate-500 hover:border-white/10 hover:text-slate-300'
                    }`}
            >
                {label}
            </button>
