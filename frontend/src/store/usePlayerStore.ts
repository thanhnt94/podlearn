...
interface PlayerState {
  ...
  setActiveTrackId: (id: number | string | null) => void;
  ...
  // UI / Interaction
  mode: 'watch' | 'shadowing';
  isVideoLooping: boolean;
  setIsVideoLooping: (isLooping: boolean) => void;
  isLoaded: boolean;
  isCompleted: boolean;
  isLocked: boolean;
  ...
  isWordMapLoading: boolean;
  vocabEditorMode: 'db' | 'sub';
  setVocabEditorMode: (mode: 'db' | 'sub') => void;
  ...
  setMode: (mode: 'watch' | 'shadowing') => void;
  ...
  // Word Map Actions
  fetchWordMap: (trackId?: number | string) => Promise<void>;
  updateWordStatus: (lemma: string, status: 'use' | 'skip') => Promise<void>;
}
...
export const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  rawJsonInput: '',
  vocabEditorMode: 'db',
  setVocabEditorMode: (mode) => {
    set({ vocabEditorMode: mode });
    const { lessonId } = get();
    if (lessonId) {
      axios.patch(`/api/study/lesson/${lessonId}/settings`, {
        vocab_editor_mode: mode
      }).catch(err => console.error("Failed to persist vocab mode:", err));
    }
  },
  ...
  setIsVideoLooping: (isLooping) => set({ isVideoLooping: isLooping }),
  isLoaded: false,
  isCompleted: false,
  isLocked: false,
  lockMessage: null,
  ...
  setLessonData: (data) => set((state) => {
    ...
    if (data.settings?.vocab_editor_mode !== undefined) vocabSettings.vocabEditorMode = data.settings.vocab_editor_mode;
  ...
  fetchWordMap: async (trackId?: number | string) => {
    const { lessonId } = get();
    if (!lessonId) return;
    set({ isWordMapLoading: true });
    try {
      const url = trackId ? `/api/study/vocab/word-map/${lessonId}/${trackId}` : `/api/study/vocab/word-map/${lessonId}`;
      const res = await axios.get(url);
      set({ wordMap: res.data.words || [] });
    } catch (e) {
      console.error("Failed to fetch word map", e);
    } finally {
      set({ isWordMapLoading: false });
    }
  },

  updateWordStatus: async (lemma, status) => {
    const { lessonId, fetchWordMap, fetchAnalyzedWords, subtitles, activeLineIndex, originalLang } = get();
    if (!lessonId) return;
    try {
      await axios.post('/api/study/vocab/word-map/update-status', { lesson_id: lessonId, lemma, status });
      // Refresh word map with the current track context
      await fetchWordMap(get().activeTrackId || undefined);
      // Refresh current line analysis to reflect skip/use change
      if (activeLineIndex !== -1 && subtitles[activeLineIndex]) {
        await fetchAnalyzedWords(subtitles[activeLineIndex].text, originalLang);
      }
    } catch (e) {
