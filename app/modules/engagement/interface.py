...
def get_user_notifications_dto(user_id: int, limit: int = 20) -> List[Dict[str, Any]]:
    with SessionLocal() as db:
        notifs = db.query(Notification).filter_by(user_id=user_id).order_by(Notification.created_at.desc()).limit(limit).all()
        return [{
            'id': n.id,
            'type': n.type,
            'title': n.title,
    ...
    with SessionLocal() as db:
        ...
        for b in all_badges:
            ...
            if b.id in earned_ids:
                ub = db.query(UserBadge).filter_by(user_id=user_id, badge_id=b.id).first()
...
def get_pending_shares_dto(user_id: int) -> List[Dict[str, Any]]:
    with SessionLocal() as db:
        pending_shares = db.query(ShareRequest).filter_by(receiver_id=user_id, status='pending').all()
        return [{
            'id': s.id,
...
def get_stats_summary_dto(user_id: int) -> Dict[str, Any]:
    ...
    with SessionLocal() as db:
        ...
        total_shadowing_duration = db.query(func.sum(ActivityLog.duration_seconds)).filter(
            ActivityLog.user_id == user_id,
    ...
    with SessionLocal() as db:
        ...
        for s in stats:
            results[str(round(float(s.start_time), 3))] = {
                ...
                'avg': int(s.avg_score or 0),
                'best': s.best_score or 0
            }
        return results
...
def award_badge_async(user_id: int):
    with SessionLocal() as db:
        user = db.query(User).get(user_id)
        if user:
            return GamificationService.check_and_award_badges(user, db=db)
    return []

def accept_share_request(share_id: int, user_id: int) -> bool:
    with SessionLocal() as db:
        share = db.query(ShareRequest).filter_by(id=share_id, receiver_id=user_id, status='pending').first()
        if not share:
            return False
...
def get_video_id_from_share(share_id: int) -> Optional[int]:
    with SessionLocal() as db:
        share = db.query(ShareRequest).get(share_id)
...
def get_user_stats_dto(user_id: int) -> Dict[str, Any]:
    with SessionLocal() as db:
        ...
        return {
            ...
            'completed_count': getattr(user, 'completed_count', 0),
...
def evaluate_pronunciation_manual(user_id: int, lesson_id: int, original_text: str, spoken_text: str, lang: str = 'ja', sentence_id: Optional[int] = None):
    """
    Manual scoring for pronunciation without background tasks.
    """
    from app.modules.engagement.services.shadowing_service import evaluate_pronunciation
    # Actually I need to implement evaluate_pronunciation first or find it.
    # For now, return a dummy score.
    score = 85 # Placeholder
    from app.modules.engagement.events import handle_shadowing_completed
    # video_id lookup...
    with SessionLocal() as db:
