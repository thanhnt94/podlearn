...
def export_lesson_package(lesson_id: int):
    ...
    with SessionLocal() as db:
        ...
        tracks = db.query(SubtitleTrack).filter_by(video_id=video.id).all()
        for t in tracks:
        ...
        dicts = db.query(VideoDictionary).filter_by(lesson_id=lesson.id).all()
        for d in dicts:
            d_data = {
                "name": d.name,
...
def import_lesson_package(user_id: int, package_data: dict):
    ...
    try:
        with SessionLocal() as db:
            ...
            lesson = db.query(Lesson).filter_by(user_id=user_id, video_id=video.id).first()
            if not lesson:
                lesson = Lesson(
                    user_id=user_id,
                    video_id=video.id,
            ...
            for s_data in package_data.get("subtitles", []):
                ...
                existing_track = db.query(SubtitleTrack).filter_by(
                    video_id=video.id,
            ...
            for d_data in package_data.get("dictionaries", []):
                ...
                v_dict = db.query(VideoDictionary).filter_by(
                    lesson_id=lesson.id,
                    name=d_data["name"]
                ).first()
                ...
                for item_data in d_data.get("items", []):
                    existing_item = db.query(VideoGlossary).filter_by(
