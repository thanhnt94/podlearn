# PodLearn - Các Tính năng và Chế độ Hoạt động

PodLearn là một nền tảng toàn diện cho việc tiếp nhận ngôn ngữ thông qua nội dung video. Ứng dụng cung cấp nhiều chế độ chuyên biệt giúp người học chuyển từ nghe hiểu thụ động sang sản xuất ngôn ngữ chủ động.

## 1. Trung tâm Học tập (Dashboard V3)
Giao diện tập trung để quản lý lộ trình học tập của bạn.
- **Thư viện Cá nhân**: Xem tất cả video đã nhập và các bài học đang thực hiện.
- **Bộ sưu tập (Library Sets)**: Tạo và quản lý các danh sách phát (Playlists) riêng tư hoặc công khai để phân loại video theo chủ đề hoặc cấp độ.
- **Khám phá Video**: Gợi ý các nội dung công khai từ cộng đồng.
- **Bảng Thống kê**: Theo dõi chuỗi ngày học (Streak), tổng thời gian học và tiến độ hoàn thành.
- **Thông báo**: Nhận và phản hồi các yêu cầu chia sẻ bài học từ bạn bè.

## 2. Trình phát Video Tương tác (Shadowing Studio)
Môi trường cốt lõi nơi việc học diễn ra.
- **Phụ đề Thông minh**: Hỗ trợ hiển thị đồng thời phụ đề gốc, phụ đề dịch và phụ đề ngôn ngữ thứ ba.
- **Tra cứu Tức thì**: Click vào bất kỳ từ nào trong phụ đề để tạm dừng video, nghe phát âm và xem định nghĩa chi tiết.
- **Điều hướng Câu Chính xác (Sentence Navigation)**: 
    - Nhảy chính xác đến câu tiếp theo hoặc quay lại đầu câu/câu trước đó chỉ bằng một cú click.
    - Logic thông minh: Tự động quay lại đầu câu hiện tại nếu đã nghe quá 1.2 giây, giúp luyện nghe lặp đi lặp lại cực nhanh.
    - Giữ nguyên trạng thái Pause khi nhảy câu để ghi chú hoặc tra cứu.
- **Shadowing (Luyện nói ngắt quãng)**: 
    - Tự động tạm dừng sau mỗi câu để người học lặp lại.
    - Chấm điểm phát âm bằng AI và lưu lại lịch sử độ chính xác.
- **AI Insights & Deep Analysis (Phân tích chuyên sâu 8 lớp)**:
    - **On-demand Analysis**: Phân tích từng câu riêng lẻ theo yêu cầu người dùng để tối ưu hiệu suất và dung lượng.
    - **8-Card Carousel**: Giao diện thẻ lật cung cấp 8 khía cạnh kiến trúc ngôn ngữ:
        1. **Dịch nghĩa**: Bản dịch tự nhiên kèm tóm tắt ý nghĩa.
        2. **Ngữ pháp**: Giải thích chi tiết cấu trúc và các điểm ngữ pháp quan trọng.
        3. **Từ vựng**: Liệt kê 3-5 từ khóa kèm ý nghĩa và cách dùng trong câu.
        4. **Sắc thái**: Giải thích tông giọng (lịch sự/thân mật), phong cách (văn nói/viết).
        5. **Câu tương tự**: Đề xuất 3-4 biến thể khác để mở rộng vốn câu.
        6. **Văn hóa**: Bối cảnh sử dụng, phép lịch sự hoặc các lưu ý văn hóa liên quan.
        7. **Mẹo nhớ**: Các phương pháp ghi nhớ nhanh và sáng tạo.
        8. **Lỗi thường gặp**: Cảnh báo các sai lầm phổ biến khi dùng cấu trúc này.
    - **Shadowing Flow Sync**: Tự động tạm dừng video để người học lật thẻ nghiên cứu trước khi qua câu tiếp theo.
- **Ghi chú Dòng thời gian**: Chụp lại các ý tưởng hoặc cấu trúc hay ngay tại thời điểm xuất hiện trong video.

## 3. Các Chế độ Học Mastery (Ôn tập)
Sau khi các câu hoặc mẫu câu được lưu lại, chúng có thể được ôn tập chuyên sâu.

### A. Sentence Mastery (Làm chủ câu)
- Tập trung vào cấu trúc toàn diện của câu.
- **Luyện viết**: Gõ lại câu dựa trên âm thanh và nghĩa.
- **Luyện nói**: Nghe và lặp lại các câu đã lưu một cách cô đọng.

### B. Grammar Mastery (Làm chủ Ngữ pháp)
- Đi sâu vào các quy tắc cụ thể (vd: "Đảo ngữ", "Giả định cách").
- **Phân tích Cấu trúc**: Xem giải thích chi tiết tại sao cấu trúc đó được sử dụng.
- **Nhận diện Mẫu**: Luyện tập nhận diện và sử dụng cùng một điểm ngữ pháp trong các ngữ cảnh khác nhau.

### C. Vocabulary Mastery (Làm chủ Từ vựng)
- Tập trung vào các từ đơn lẻ trong ngữ cảnh của câu gốc.
- **Hệ thống Ôn tập Ngắt quãng (SRS)**: Thuật toán Adaptive Flow giúp tối ưu hóa thời gian ghi nhớ.
- **Sử dụng Ngữ cảnh**: Xem tất cả các câu từ lịch sử học tập nơi từ đó xuất hiện.

## 4. Quản lý Nội dung & Wiki Cộng đồng
- **Nhập từ YouTube**: Nhập bất kỳ video YouTube nào qua URL. Hệ thống tự động lấy phụ đề có sẵn.
- **Tùy chỉnh Phân tách từ**: Chỉnh sửa tay cách tách câu (segmentation) cho các ngôn ngữ phức tạp như tiếng Nhật.
- **Collaborative Wiki**: Xây dựng bộ từ điển (Glossary) chung cho video cùng cộng đồng người học.

## 5. Gamification (Trò chơi hóa)
- **Hệ thống Streak**: Khuyến khích thói quen học tập hàng ngày.
- **Mục tiêu Cá nhân**: Thiết lập mục tiêu thời gian học hoặc số lượng cấu trúc mới mỗi ngày.

## 6. Bảng Quản trị (Admin Panel)
Dành cho người quản trị để quản lý tài nguyên hệ thống.
- **Quản lý Người dùng**: Theo dõi hoạt động và phân quyền.
- **Điều phối Video**: Sửa chữa metadata hoặc quản lý các bản phụ đề lỗi.
- **Giám sát Hệ thống**: Kiểm tra trạng thái kết nối với các dịch vụ bên ngoài (CentralAuth, YouTube API).
