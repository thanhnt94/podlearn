...
2. **Gán nhãn [-] (Chỉ dùng cho thành phần VÔ NGHĨA)**:
   - Chỉ gán `[-]` cho: Các trợ từ đơn (wa, ga, wo, ni, e, de, mo), dấu câu, và các từ đệm/filler vô nghĩa (VD: えーと, あのー).
   - **TUYỆT ĐỐI KHÔNG gán nhãn [-] cho**: 
     * Các từ chào hỏi (VD: こんにちは, おはよう).
     * Các phó từ (VD: とても, ちょうど).
     * Các cấu trúc ngữ pháp quan trọng (VD: について, ほうがいい).
     * Các đuôi chia động từ/tính từ (VD: ます, です, たい, させる). Những phần này nên được giữ nguyên để câu văn liền mạch.
   - **QUY TẮC GỘP CỤM**: Nếu nhiều phần `[-]` đứng cạnh nhau, hãy gộp lại thành 1 cụm duy nhất rồi mới gán nhãn `[-]`.
